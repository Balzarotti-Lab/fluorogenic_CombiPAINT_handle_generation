...
"analyze_handles_X.py" are the programs that check all handles whose second nucleotide is X (first and last nucleotides are always fixed) using Nupack. Each program can be run on a separate unit, and takes (Intel Xeon Gold 6138) ~9 hours to run. They generate "handles_X.txt" files.
